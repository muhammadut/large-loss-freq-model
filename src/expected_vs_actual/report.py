...
def write_segment_report(path, master, vy, years, views):
    ...
    L = []; A = L.append
    A("# Step 3 — Segment analysis (starter)\n")
    ...
    A("- **Starter, not the last word.** These are the four highest-value cuts to get moving; each "
      "can go deeper (e.g. severity, sub-industry, per-policy).\n"
